...
class RecruitPostCategoryVC: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    let categoryNames = ["Birthday In The Box", "Peduli Tak Pernah Libur", "Peduli Edukasi"]
    
    let ageGroups = ["5", "10", "15", "20", "25", "30"]
    ...
    override func viewDidLoad() {
        ...
        if isPickerCategory == true{
            ...
            titleLabel.text = "Select Needed Quota"
        }
        
        if selectedCategory.count > 0 && isPickerCategory == true{
            pickerView.selectRow(categoryNames.firstIndex(of: selectedCategory)!, inComponent: 0, animated: true)
        }
        
        if selectedAgeGroup.count > 0 && isPickerCategory == false{
            pickerView.selectRow(ageGroups.firstIndex(of: selectedAgeGroup)!, inComponent: 0, animated: true)
        }
